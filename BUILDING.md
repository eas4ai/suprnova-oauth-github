...
This tutorial builds a GitHub OAuth provider as an ordinary third-party Suprnova developer. The finished crate lives outside the Suprnova workspace, depends on the public `v1.3.6` Git tag, and imports every SDK type through `suprnova::`.
...
serde_json = "1"
suprnova = { version = "=1.3.6", git = "https://github.com/eas4ai/suprnova.git", tag = "v1.3.6" }
thiserror = "2"
url = "2"
```
...
Version `0.1.4` does not opt into GitHub's optional expiring-token mode, so its refresh policy is explicit:
...
The published GitHub plugin used this sequence for `v0.1.4`. Its final suite covered provider behavior, transport behavior, real HTTP requests against a mock server, complete public-SDK identity exchange, and the dependency firewall.
