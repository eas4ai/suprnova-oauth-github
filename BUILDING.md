...
This tutorial builds a GitHub OAuth provider as an ordinary third-party Suprnova developer. The finished crate lives outside the Suprnova workspace, depends on the public `v2.0.2` Git tag, and imports every SDK type through `suprnova::`.
...
serde_json = "1"
suprnova = { version = "=2.0.2", git = "https://github.com/eas4ai/suprnova.git", tag = "v2.0.2" }
thiserror = "2"
url = "2"
```
...
Version `0.2.1` does not opt into GitHub's optional expiring-token mode, so its refresh policy is explicit:
...
The published GitHub plugin used this sequence for `v0.2.1`. Its final suite covered provider behavior, transport behavior, real HTTP requests against a mock server, complete public-SDK identity exchange, and the dependency firewall.
