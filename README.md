...
`suprnova-oauth-github` is an external GitHub OAuth provider plugin for Suprnova. It uses only the public API exported by `suprnova` v1.3.4. It doesn't depend directly on `suprnova-magnetar`, use a Suprnova workspace path, or access framework internals.
...
suprnova-oauth-github = { git = "https://github.com/eas4ai/suprnova-oauth-github.git", tag = "v0.1.2" }
url = "2"
```
...
GitHub deletes the application grant and all OAuth tokens associated with that user. The plugin doesn't enable GitHub's optional expiring-token and refresh-token mode in v0.1.2.
...
The `public_sdk_firewall` test verifies that the manifest uses the public Suprnova `v1.3.4` Git tag, has no path dependency, and has no direct Magnetar dependency or import.
